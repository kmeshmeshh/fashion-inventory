...
export default function ShipmentsPage() {
  ...
  const createShippingCity = useCreateShippingCity();
  const updateOrder = useUpdateOrder();

  const [sheetOpen, setSheetOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [cityLoading, setCityLoading] = useState(false);
  const [activeTab, setActiveTab] = useState<"shipments" | "cities">(
    "shipments",
  );
  const [newCity, setNewCity] = useState({ city_name: "", shipping_fee: "" });
  ...
  const handleCreateShippingCity = async () => {
    const cityName = newCity.city_name.trim();
    const fee = Number(newCity.shipping_fee);
    if (!cityName) {
      alert("يرجى إدخال اسم المدينة");
      return;
    }
    if (Number.isNaN(fee) || fee < 0) {
      alert("يرجى إدخال قيمة شحن صحيحة");
      return;
    }

    setCityLoading(true);
    try {
      await createShippingCity.mutateAsync({
        city_name: cityName,
        shipping_fee: fee,
      });
      setNewCity({ city_name: "", shipping_fee: "" });
    } catch (err) {
      alert("❌ " + (err instanceof Error ? err.message : "حدث خطأ"));
    } finally {
  ...
  const orderShippingFee = (order: any) =>
    order?.shipping_city?.shipping_fee ??
    order?.shipping_cities?.shipping_fee ??
    0;
  ...
  const totalShippingFee = (shipment: any) =>
    shipment.shipment_orders?.reduce(
      (s: number, so: any) => s + orderShippingFee(so.orders),
      0,
    ) || 0;
                  ...
                  {readyForCourier.map((order: any) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        {orderShippingFee(order) > 0 && (
                          <Badge
                            variant="outline"
                            className="text-[10px] border-amber-500/20 text-amber-300 gap-1"
                          >
                            توصيل: {fmt(orderShippingFee(order))} EGP
                          </Badge>
                        )}
                ...
                >
                  <CardContent className="p-4">
                    ...
                    <div className="flex flex-wrap items-center gap-3 mb-3">
                      ...
                      <Badge
                        variant="outline"
                        className="text-[10px] border-amber-500/20 text-amber-300 tabular-nums"
                      >
                        توصيل: {fmt(totalShippingFee(shipment))} EGP
                      </Badge>
                    </div>
                    {shipment.shipment_orders?.map((so: any) => (
                      <div
                        key={so.order_id}
                        className="flex flex-col gap-3 rounded-xl border border-zinc-800/40 bg-zinc-950/40 p-3 text-xs"
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            {so.orders.shipping_city?.city_name && (
                              <p className="text-[11px] text-amber-300">
                                مدينة الشحن: {so.orders.shipping_city.city_name}
                              </p>
                            )}
                          </div>
                          <Select
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {orderShippingFee(so.orders) > 0 && (
                            <Badge
                              variant="outline"
                              className="text-[10px] border-amber-500/20 text-amber-300 gap-1"
                            >
                              توصيل: {fmt(orderShippingFee(so.orders))} EGP
                            </Badge>
                          )}
                        </div>
                      </div>
      ...
      {activeTab === "cities" && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
            <Card className="bg-zinc-900 border-zinc-800">
              <CardContent className="p-4">
                <p className="text-xs text-zinc-500">عدد المدن</p>
                <p className="text-2xl font-bold text-white tabular-nums">
            ...
            <Card className="bg-zinc-900 border-zinc-800 lg:col-span-2">
              <CardContent className="p-4">
                <p className="text-xs text-zinc-500">أعلى رسوم شحن</p>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
              ...
              <CardContent className="p-4">
                ...
                <div className="space-y-3">
                  <div className="space-y-1.5">
                    <Label className="text-zinc-400 text-xs">اسم المدينة</Label>
                    <Input
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setNewCity({ ...newCity, shipping_fee: e.target.value })
                      }
                      className="bg-zinc-800 border-zinc-700 text-white placeholder:text-zinc-600"
                    />
                  </div>
                  <Button
                    onClick={handleCreateShippingCity}
                    disabled={cityLoading}
                    className="w-full bg-indigo-600 hover:bg-indigo-500 text-white"
                  >
                    {cityLoading ? "جاري الإضافة..." : "إضافة المدينة"}
                  </Button>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-zinc-900 border-zinc-800">
              <CardContent className="p-4">
                ...
                {Array.isArray(shippingCities) && shippingCities.length ? (
                  <div className="space-y-2">
                    {shippingCities.map((city: any) => (
                      <div
                        key={city.id}
                        className="flex items-center justify-between rounded-xl border border-zinc-700/50 bg-zinc-800/50 px-4 py-3"
                      >
                        <div>
                          ...
                          <p className="text-[11px] text-zinc-500">
                            رسوم الشحن: {fmt(city.shipping_fee)} EGP
                          </p>
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="text-zinc-500 text-sm">
                    لا توجد مدن شحن بعد. أضف أول مدينة باستخدام النموذج.
                  </div>
                )}
              </CardContent>
            </Card>
